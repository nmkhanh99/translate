...
16. **ENGINE RENDER V2 (2026-07-13, học từ PDFMathTranslate/BabelDOC)** — thay toàn bộ đường vẽ `insert_textbox` 1-style bằng **rich-text `insert_htmlbox` + đo trước bằng `fitz.Story`**: (a) **đậm/nghiêng/chỉ-số-trên inline giữ qua bản dịch** bằng marker `<b>/<i>/<sup>` trong text segment (trước đây 1 style/segment → mất đậm term, mất nghiêng *most likely*/*not* trong câu hỏi thi); (b) font 4 mặt Times New Roman (regular/bold/italic/bold-italic, `find_font_family`, override `CFA_TRANSLATE_FONT[_BOLD|_ITALIC|_BOLDITALIC]`); (c) **canh đều (justify) như sách** khi đoạn nguồn justify (`_seg_align`), giãn dòng đo từ nguồn (`_seg_lh`); (d) **chuẩn hoá cỡ chữ toàn tài liệu**: đo scale vừa khung cho từng đoạn bằng Story (không vẽ), lấy mode theo trọng số độ dài, ép trần chung (không ép dưới 0.85) → hết "cỡ chữ nhảy lung tung giữa các đoạn"; (e) lưới an toàn `scale_low=0.3` → không bao giờ tràn đè phần tử giữ nguyên (trước đây co tới 5.5pt rồi vẽ tràn). Render vẫn **deterministic** (golden pixel-hash dùng tiếp được — có unit test).
17. **Công thức inline trong prose hết vỡ** (`congthuc_vo` — cụm lỗi lớn nhất còn lại): run công thức ở mức SPAN trong dòng prose (`_span_is_mathish`: superscript-flag/lệch cỡ >25%/ký hiệu toán-Hy Lạp-combining; `_span_attachable` dính `P`/`(1 +` kề bên) → thay bằng marker `{vN}` trong text, rect nguồn lưu vào `layout.fx`; khi apply, **vùng gốc được raster 288dpi từ trang CÒN NGUYÊN và nhúng lại dưới dạng ảnh inline** đúng vị trí trong dòng chữ Việt reflow. Ký hiệu X̄/R̄ (combining macron từng render thành `_`), σ², P(A|B), sub/superscript... giữ nguyên hình dạng gốc. **Hàng rào chống LLM phá marker**: `check_markers` (unescape `&lt;i&gt;`, chuẩn hoá `{ v1 }`/`(v1)`/`（v1）`, thẻ hỏng cặp bị strip; mất/lệch `{vN}` → LOẠI bản dịch, giữ tiếng Anh, log `marker_drop.json`) chạy ở merge-tr/merge-vr/merge-fix; prompt translate/verify/fix có luật marker; verify được dặn soát marker lệch.
18. **Bảng 3+ cột không nhãn đậm hết vỡ** (`bang_vo` — trước đây "không fix được vì FP cao"): `_page_grid_keys` nhận diện bảng ở mức TRANG theo cấu trúc thật của sách (mỗi Ô là một LINE riêng cùng hàng y): hàng ≥3 ô, vùng ≥3 hàng liên tiếp, ≥2 cột **thẳng hàng theo TÂM ô** (cột canh giữa/phải) → mọi dòng trong vùng giữ nguyên tiếng Anh + làm boundary kẹp đáy/redact. Đo trên v1: 127 trang có vùng bảng được bảo vệ (5734 dòng), bắt 15/30 trang cụm bang_vo, prose/bullet/TOC không dính (kiểm tra tay p18: đúng bảng 5 cột Investment/Liquidity/Default-Risk; unit test 4 case). Ô bảng lẻ từng bị dịch rời (`146 days`, `Correlation`...) nay đứng yên trong bảng.
19. **Glossary hook**: đặt `glossary.json` (`{"english term": "bản dịch"}`) vào workdir → runner tự chèn vào prompt dịch + verify (≤80 mục) để thuật ngữ nhất quán toàn volume. Không có file → như cũ.

**Kiểm chứng render v2 (2026-07-13):** unit test 27/27 pass (`test_engine_v2.py`, gồm determinism end-to-end); sentinel extraction-diff 34 trang v1: box/size delta = 0 toàn bộ (thay đổi duy nhất là marker trong text); apply full v1 (624 trang) 5.5 phút, deterministic; **chấm mù 8 trang đại diện** (ảnh ghép GỐC|CŨ|MỚI, 1 agent/trang): **8/8 bản mới tốt hơn, 0 ngang, 0 tệ hơn** — p31/42/68/235 hết sạch defect (bảng EUR + Exhibit 16 thẳng cột, công thức (9) nguyên vẹn, hết dòng 'dep' lẻ), p20/25 giữ được công thức (2)/phân số/superscript mà bản cũ làm vỡ. Lỗi còn lại đều là pre-existing chung 2 bản (X̄_H đè chữ đậm p25, mất highlight vàng = policy) hoặc hạn chế đã ghi nhận (ảnh {vN} nhô baseline ~2-4pt).

> Sau MỖI lần sửa engine: chạy `apply-all volumes.json` để bake vào tất cả PDF (nên gom nhiều fix rồi apply-all 1 lần).
> **NGOẠI LỆ — fix #16/#17 (engine v2) đổi TEXT segment** (thêm marker `<b>/<i>/<sup>/{vN}`) → ~27%
> segment đổi key cache (đo trên v1: 1310/4819) → quy trình mỗi volume: `chunk --force` → dịch phần
> `todo` mới → `merge-tr` → `apply` (giống ngoại lệ fix #9/... bên dưới). Segment không có style/công
> thức inline giữ nguyên key → cache hit như cũ.
...
**Còn lại không fix**: đánh đổi nền tảng "tiếng Việt dài hơn ~30% → co chữ/nhồi sát" (low/medium — v2 đã giảm nhờ chuẩn hoá scale + justify nhưng bản chất còn); ~~công thức nhúng trong prose vỡ~~ → **ĐÃ FIX #17** (ảnh inline {vN}; hạn chế nhỏ còn lại: ảnh đặt đáy-trên-baseline nên cụm có subscript hơi nhô cao ~2-4pt, Story không hỗ trợ vertical-align); ~~bảng 3+ cột không nhãn đậm~~ → **ĐÃ FIX #18** (bắt 15/30 trang bang_vo; các trang còn lại là dạng 2-cột/1-hàng chưa đủ bằng chứng hình học, xem tiếp sau vision); highlight vàng mất khi redact (fix #5 đã chấp nhận, xem mục 4.4). Còn 2 pattern CHƯA điều tra xong ở mục 4.4 (bullet đè chữ, cụm đậm giữa câu bị nhận nhầm heading — cụm đậm giữa câu có thể đã giảm nhờ `<b>` inline của v2, cần vision xác nhận).
