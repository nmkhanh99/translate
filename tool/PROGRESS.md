...
```bash
cd tool && python3 agent_pipeline.py batch-status volumes.json
```
...
```bash
cd tool && python3 agent_pipeline.py apply-all volumes.json
```
...
- `tool/pdf_core.py` — engine trích đoạn + ghi đè giữ layout (15 fix ở mục 6).
- `tool/agent_pipeline.py` — glue: chunk/merge/apply/status/pending/vis/problems/revision/volumes/apply-all.
- `tool/translate_volume.js` — Workflow 4 phase, resume + batch + `only:'vision'` + vision theo cửa sổ.
- `tool/volumes.json` — manifest 10 tài liệu (pdf → workdir → out).
