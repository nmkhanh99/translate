...
### B. Dịch tự động hàng loạt (CLI, Google Translate free) — Design B
```bash
python3 translate_pdf.py "INPUT.pdf" --out OUT.pdf --pages 40-46
```
Có cache JSON → chạy lại miễn phí, resume được. Chất lượng = Google Translate.

### C. Pipeline AGENT đầy đủ 1 volume, RESUME được — `translate_volume.js` (Workflow)
...
**Chạy 1 volume:**
```js
Workflow({ scriptPath: "/Users/khanhnm/Desktop/translate/tool/translate_volume.js",
  args: { pdf: "<source.pdf>", workdir: "<tool>/work/v3",
          out: "<đích.pdf>", tool: "/Users/khanhnm/Desktop/translate/tool",
...
```js
Workflow({ scriptPath: "/Users/khanhnm/Desktop/translate/tool/translate_volume.js",
  args: { tool: "/Users/khanhnm/Desktop/translate/tool" } })   // không có pdf -> batch
```
...
# --- vòng lặp review per-page (xem mục C ở trên) ---
...
### D. Màn hình quản lý (web) — `dashboard.py`

Không muốn gõ lệnh Workflow tay: mở dashboard để **xem tiến độ mọi volume** và
**bấm Chạy/Dừng**. Chọn được **Engine** cho mỗi lần chạy:
- **Claude** — spawn `claude -p` chạy luồng C (Workflow 4-phase), resume được.
- **Codex** — spawn `codex exec` dùng MCP dịch cả volume theo **lô trang** nối
  chuỗi (Design A mở rộng); nhanh, gọn, không có verify/vision. Resume nhẹ bằng
  `codex_state.json` + `codex_work.pdf` trong workdir.

Chỉ dùng thư viện chuẩn, không cần Flask.

```bash
python3 dashboard.py            # mở http://127.0.0.1:8756
```

Chi tiết (nút, engine, quyền spawn, file sinh ra): xem **`DASHBOARD.md`**.

### E. App macOS (Electron) — `app/`

Đóng gói dashboard thành **app macOS native** (tham khảo kiến trúc
[open-design](https://github.com/nexu-io/open-design): agent-native, spawn CLI
có sẵn). App tự khởi động `dashboard.py` rồi hiển thị trong cửa sổ; chọn engine
**Claude/Codex** ngay trong app.

```bash
cd app && npm install && npm start
```

Chi tiết: xem **`app/README.md`**.
...
### Claude Code (CLI)
```bash
claude mcp add cfa-pdf-translator -- python3 /Users/khanhnm/Desktop/translate/tool/server.py
```
...
| `server.py` | MCP server (Design A). |
| `translate_pdf.py` | CLI dịch tự động 1 file (Design B, Google). |
| `translate_all.py` | Dịch HÀNG LOẠT cả thư mục (song song + cache resume). |
...
| `dashboard.py` | **Màn hình quản lý (web)**: xem tiến độ + Chạy/Dừng pipeline từng volume/batch. Xem `DASHBOARD.md`. |
| `volumes.json` | Manifest 10 tài liệu (pdf → workdir → out). |
| `requirements.txt` | Phụ thuộc. |

## Dịch hàng loạt cả thư mục

```bash
python3 translate_all.py \
  --src "2024 CFA L1 Curriculum" \
  --out "2024 CFA L1 Curriculum - VI" --workers 12
```
- Dịch song song nhiều luồng; ghi `_progress.log` để theo dõi.
- Cache JSON / mỗi PDF → resume được. **Sửa tool rồi chạy lại chỉ tốn bước
  apply** (bản dịch đã cache, không gọi lại mạng) → vá layout cho cả bộ rất nhanh.
- Lỗi 1 đoạn → giữ nguyên tiếng Anh (KHÔNG cache) để lần sau thử lại.
