...
Quy trình 1 volume:
  ...
  8) pairs   : render ảnh ghép trang gốc|dịch (review/) cho Workflow review
...
def cmd_vchunk(pdf, workdir, size=25, mode="all", force=False):
    ...
    for s in segs:
        ...
        if en in seen:
            continue
        vi = text2vi.get(en, "")
        if mode == "all" or DIGIT.search(en) or untранслated(en, vi):
            seen.add(en)
...
def cmd_pairs(pdf, out, workdir, dpi=130):
    rev = _wd(workdir, "review")
    ...
    gap = 20
    for i in range(src.page_count):
        p1 = src[i].get_pixmap(matrix=m)
        p2 = vi[i].get_pixmap(matrix=m)
        W, H = p1.width + gap + p2.width, max(p1.height, p2.height)
        d = fitz.open()
        pg = d.new_page(width=W, height=H)
        pg.insert_image(fitz.Rect(0, 0, p1.width, p1.height), pixmap=p1)
        pg.insert_image(fitz.Rect(p1.width + gap, 0, p1.width + gap + p2.width, p2.height), pixmap=p2)
        pg.get_pixmap(matrix=fitz.Matrix(1, 1)).save(_wd(rev, f"pair_{i:03d}.png"))
    print(f"pairs={src.page_count} dir={rev}")
...
if __name__ == "__main__":
    ...
    a = [x for x in a if x != "--force"]
    {
        ...
        "pairs": lambda: cmd_pairs(a[0], a[1], a[2]),
